...
class ComputerController extends Controller
{
    public function index(Request $request) // Display a listing of the resource
    {
      ...
      if (!$searchQueryValidator -> fails())
      {
        ...
        $columns = ['id', 'mainImage', 'title', 'price', 'discount', 'conditionId', 'stockTypeId', 'visibility', 'isOffer', 'uuid', 'warrantyDuration', 'warrantyId'];
      ...
      return \View::make('contents.user.computers.index') -> with([
    ...
    public function store(StoreComputerRequest $request) // Store a newly created resource in storage
    {
      ...
      if ($request -> file('mainImage') -> isValid())
      {
        ...
        if ($fileName)
        {
          ...
          $codeExists = \DB::table('computers') -> where('code', $data['code']) -> count();
    ...
    public function edit($id) // Show the form for editing the specified resource
    {
      try{
        ...
        return \View::make('contents.user.computers.edit') -> with([
    ...
    public function update(UpdateComputerRequest $request) // Update the specified resource in storage
    {
      $data = $request -> validated();
