...
class Accessory extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange)
          {
            ...
            if($accessoryTypesExist && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = ['accessories.id', 'title', 'mainImage', 'discount', 'price'];
              $accessoryTypesNumbers = $conditionNumbers = $stockTypesNumbers = [];

              $query = \DB::table('accessories') -> select($columns) -> where('visibility', 1);
              ...
              if($currentPage != 0 && $totalNumOfProducts != 0)
              {
                ...
                $data['maxPage'] = $maxPageNumber;
                $data['pages'] = $paginator -> build($totalNumOfProducts, 3, $numOfProductsToView, $currentPage, 2, 0) -> get();
    ...
    public function getAccessoriesForHomePageData($id)
    {
       $categoryId = (int) $id;
    ...
    public function getAccessoriesData($categoryId)
    {
      $numOfProductsToView = 9;
      $adjacent = 3;
      $currentPage = 1;
      ...
      if($data['configuration']['accessoriesTypesExist'])
      {
        ...
        if($data['configuration']['productPriceRangeExists'])
        {
          ...
          $columns = ['accessories.id', 'title', 'mainImage', 'price', 'discount', 'code'];

          $query = \DB::table('accessories') -> select($columns) -> where('visibility', 1) -> where('price', '>=', $productMinPrice) -> where('price', '<=', $productMaxPrice);

          if($categoryId != 0) $query = $query -> where('accessoryTypeId', '=', $categoryId);

          $data['accessories'] = $query -> take($numOfProductsToView) -> get();
          ...
          if($data['accessoriesExist'])
          {
            ...
            $data['accessories'] = $data['accessories'] -> chunk($chunkSize);
          }
        }
      }
    ...
    public function getAccessoryData($id, $seoFields)
    {
      $columns = ['accessories.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'seoKeywords', 'seoDescription', 'quantity'];
      ...
      {
        $seoFields -> description = $data['accessory'] -> seoDescription;
        $seoFields -> keywords = $data['accessory'] -> seoKeywords;
        $seoFields -> title = $data['accessory'] -> title;
        ...
        $data['accessory'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
