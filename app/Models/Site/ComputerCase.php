...
class ComputerCase extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange)
          {
            ...
            if($caseFormFactorsExist && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = '`code`,`title`,`mainImage`,`discount`,`price`,`caseId`,GROUP_CONCAT(`formFactorTitle` SEPARATOR ", ") AS `formFactorTitle`';
    ...
    public function getComputerCasesData()
    {
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $totalNumOfProducts = \DB::table('computer_cases') -> where('visibility', 1) -> where('price', '>=', $productMinPrice) -> where('price', '<=', $productMaxPrice) -> count();

        $data['casesExist'] = $totalNumOfProducts != 0;
        ...
        if($data['casesExist'])
        {
          ...
          $data['cases'] = \DB::table('cases_and_form_factors') -> select(\DB::raw('caseId,code,title,mainImage,discount,price,GROUP_CONCAT(`formFactorTitle` SEPARATOR ", ") AS `formFactorTitle`'))
                                                                -> join('case_form_factors', 'case_form_factors.id', '=', 'cases_and_form_factors.formFactorId')
                                                                -> join('computer_cases', 'computer_cases.id', '=', 'cases_and_form_factors.caseId')
                                                                -> where('visibility', 1)
                                                                -> where('price', '>=', $productMinPrice)
    ...
    public function getComputerCaseData($id, $seoFields)
    {
      $columns = ['computer_cases.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
      ...
      if($data['caseExists'])
      {
        $seoFields -> description = $data['case'] -> seoDescription;
        $seoFields -> keywords = $data['case'] -> seoKeywords;
        $seoFields -> title = $data['case'] -> title;
        ...
        $data['case'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);

        $percent = $data['case'] -> newPrice * $pricePart;
        $leftRange = (int) ($data['case'] -> newPrice - $percent);
        $rightRange = (int) ($data['case'] -> newPrice + $percent);
        $fields = 'caseId,code,title,mainImage,discount,price,GROUP_CONCAT(`formFactorTitle` SEPARATOR ", ") AS `formFactorTitle`';
