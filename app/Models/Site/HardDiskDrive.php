...
class HardDiskDrive extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange)
          {
            ...
            if($formFactorsExist && $capacitiesExist && $rpmsExist && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = ['hard_disk_drives.id', 'title', 'mainImage', 'discount', 'price', 'rpm', 'capacity', 'formFactorTitle'];
              $rpmNumbers = $capacityNumbers = $formFactorsNumbers = $conditionNumbers = $stockTypesNumbers = [];

              $query = \DB::table('hard_disk_drives') -> select($columns) -> join('hard_disk_drives_form_factors', 'hard_disk_drives_form_factors.id', '=', 'hard_disk_drives.formFactorId') -> where('visibility', 1);
    ...
    public function getHardDiskDrivesData()
    {
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $columns = ['hard_disk_drives.id', 'title', 'mainImage', 'discount', 'price', 'rpm', 'formFactorId', 'capacity', 'formFactorTitle'];

        $query = \DB::table('hard_disk_drives') -> select($columns) -> join('hard_disk_drives_form_factors', 'hard_disk_drives_form_factors.id', '=', 'hard_disk_drives.formFactorId') -> where('visibility', 1);

        $totalNumOfProducts = $query -> count();

        $data['hardDiskDrivesExist'] = $totalNumOfProducts != 0;
        ...
        if($data['hardDiskDrivesExist'])
        {
          ...
          $data['hardDiskDrives'] = $query -> take($numOfProductsToView) -> get();

          foreach($data['hardDiskDrives'] as $key => $value) $data['hardDiskDrives'][$key] -> newPrice = $value -> price - $value -> discount;

          $data['hardDiskDrives'] = $data['hardDiskDrives'] -> chunk($chunkSize);
        }
      }
    ...
    public function getHardDiskDriveData($id, $seoFields)
    {
       $columns = ['hard_disk_drives.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
       ...
       {
         $seoFields -> description = $data['hardDiskDrive'] -> seoDescription;
         $seoFields -> keywords = $data['hardDiskDrive'] -> seoKeywords;
         $seoFields -> title = $data['hardDiskDrive'] -> title;
         ...
         $data['hardDiskDrive'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
