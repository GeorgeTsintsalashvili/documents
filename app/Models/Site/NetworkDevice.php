...
class NetworkDevice extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange)
          {
            ...
            if($networkDevicesTypesExist && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = ['network_devices.id', 'title', 'mainImage', 'discount', 'price'];
              $networkDevicesTypesNumbers = $conditionNumbers = $stockTypesNumbers = [];

              $query = \DB::table('network_devices') -> select($columns) -> where('visibility', 1);
    ...
    public function getNetworkDevicesData($categoryId)
    {
      ...
      if($data['configuration']['networkDevicesTypesExist'])
      {
        ...
        if($data['configuration']['productPriceRangeExists'])
        {
          ...
          $columns = ['network_devices.id', 'title', 'mainImage', 'price', 'discount', 'code'];

          $query = \DB::table('network_devices') -> select($columns) -> where('visibility', 1) -> where('price', '>=', $productMinPrice) -> where('price', '<=', $productMaxPrice);

          if($categoryId != 0) $query = $query -> where('typeId', '=', $categoryId);

          $data['networkDevices'] = $query -> take($numOfProductsToView) -> get();
          ...
          if($data['networkDevicesExist'])
          {
            ...
            $data['networkDevices'] = $data['networkDevices'] -> chunk($chunkSize);
          }
        }
      }
    ...
    public function getNetworkDeviceData($id, $seoFields)
    {
      $columns = ['network_devices.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'seoKeywords', 'seoDescription', 'quantity'];
      ...
      {
        $seoFields -> description = $data['networkDevice'] -> seoDescription;
        $seoFields -> keywords = $data['networkDevice'] -> seoKeywords;
        $seoFields -> title = $data['networkDevice'] -> title;
        ...
        $data['networkDevice'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
