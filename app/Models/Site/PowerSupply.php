...
class PowerSupply extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange && $powerFrom != 0 && $powerTo != 0 && $powerFrom <= $powerTo)
          {
            ...
            if($stockTypeExists && $conditionExists && $powerRangeExists)
            {
              ...
              $columns = ['power_supplies.id', 'title', 'mainImage', 'discount', 'price', 'power'];
    ...
    public function getPowerSuppliesData()
    {
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $columns = ['power_supplies.id', 'title', 'mainImage', 'price', 'discount', 'power'];
        $query = \DB::table('power_supplies') -> select($columns)
                                              -> where('visibility', 1)
        ...
        $data['powerSuppliesExist'] = $totalNumberOfProducts != 0;
        ...
        if($data['powerSuppliesExist'])
        {
          ...
          $data['powerSupplies'] = $query -> take($numOfProductsToView) -> get();
    ...
    public function getPowerSupplyData($id, $seoFields)
    {
      $numOfProductsToView = 12;
      $pricePart = 0.2;

      $columns = ['power_supplies.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
      ...
      {
        $seoFields -> description = $data['powerSupply'] -> seoDescription;
        $seoFields -> keywords = $data['powerSupply'] -> seoKeywords;
        $seoFields -> title = $data['powerSupply'] -> title;
        ...
        $data['powerSupply'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
