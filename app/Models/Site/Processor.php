...
class Processor extends Model
{
      ...
      if(!$validator -> fails() && !is_null($priceRange))
      {
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange && $inputSpeedRangeIsCorrect)
          {
            ...
            $speedRange = \DB::table('processors') -> selectRaw('MAX(`clockSpeed`) AS `maxSpeed`, MIN(`clockSpeed`) AS `minSpeed`') -> first();
            ...
            if($cpuTechnologyProcessesExist && $cpuSocketsExist && $cpuManufacturersExist && $cpuSeriesExist && $coresExist && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = ['processors.id', 'title', 'mainImage', 'discount', 'price', 'clockSpeed', 'socketTitle', 'size', 'cores'];
    ...
    public function getProcessorsData()
    {
      $numOfProductsToView = 15;
      $adjacent = 3;
      $currentPage = 1;
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $data['processorsExist'] = $totalNumOfProducts != 0;
        ...
        if($data['processorsExist'])
        {
           ...
           $data['processors'] = $query -> take($numOfProductsToView) -> get();
    ...
    public function getProcessorData($id, $seoFields)
    {
      $defaultNumberOfProductsToShow = 12;
      $pricePart = 0.2;

      $columns = ['processors.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
      ...
      {
        $seoFields -> description = $data['processor'] -> seoDescription;
        $seoFields -> keywords = $data['processor'] -> seoKeywords;
        $seoFields -> title = $data['processor'] -> title;
        ...
        $data['processor'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
