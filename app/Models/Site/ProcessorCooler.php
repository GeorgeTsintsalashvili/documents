...
class ProcessorCooler extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange)
          {
            ...
            if($coolersSizesExist && $cpuSocketsExist && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = '`processor_coolers`.`id`,`title`,`mainImage`,`discount`,`price`,GROUP_CONCAT(`socketTitle` SEPARATOR ", ") AS `socketTitle`';
    ...
    public function getProcessorCoolersData()
    {
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $totalNumOfProducts = \DB::table('processor_coolers') -> where('visibility', 1) -> where('price', '>=', $productMinPrice) -> where('price', '<=', $productMaxPrice) -> count();

        $data['processorCoolersExist'] = $totalNumOfProducts != 0;
        ...
        if($data['processorCoolersExist'])
        {
          ...
          $columns = '`processor_coolers`.`id`,`code`,`title`,`mainImage`,`discount`,`price`,GROUP_CONCAT(`socketTitle` SEPARATOR ", ") AS `socketTitle`';

          $data['coolers'] = \DB::table('processor_coolers_and_sockets') -> selectRaw($columns)
                                                                         -> join('cpu_sockets', 'cpu_sockets.id', '=', 'processor_coolers_and_sockets.socketId')
                                                                         -> join('processor_coolers', 'processor_coolers.id', '=', 'processor_coolers_and_sockets.processorCoolerId')
                                                                         -> where('visibility', '1')
                                                                         -> where('price', '>=', $productMinPrice)
          ...
          $data['coolers'] = $data['coolers'] -> chunk($chunkSize);
        }
      }
    ...
    public function getProcessorCoolerData($id, $seoFields)
    {
      $numOfProductsToView = 12;
      $pricePart = 0.2;

      $columns = ['processor_coolers.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
      ...
      {
        $seoFields -> description = $data['cooler'] -> seoDescription;
        $seoFields -> keywords = $data['cooler'] -> seoKeywords;
        $seoFields -> title = $data['cooler'] -> title;
        ...
        $data['cooler'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
        ...
        $fields = '`processor_coolers`.`id`,`code`,`title`,`mainImage`,`discount`,`price`,GROUP_CONCAT(`socketTitle` SEPARATOR ", ") AS `socketTitle`';

        $query = \DB::table('processor_coolers_and_sockets') -> selectRaw($fields)
                                                             -> join('cpu_sockets', 'cpu_sockets.id', '=', 'processor_coolers_and_sockets.socketId')
                                                             -> join('processor_coolers', 'processor_coolers.id', '=', 'processor_coolers_and_sockets.processorCoolerId')
                                                             -> where('visibility', 1)
        ...
        $data['recommendedCoolers'] = $query -> take($numOfProductsToView) -> get();
        $data['recommendedCoolersExist'] = $query -> count() != 0;
