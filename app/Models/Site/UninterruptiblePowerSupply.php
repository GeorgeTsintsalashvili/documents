...
class UninterruptiblePowerSupply extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange && $powerFrom != 0 && $powerTo != 0 && $powerFrom <= $powerTo)
          {
            ...
            if($stockTypeExists && $conditionExists && $powerRangeExists)
            {
              ...
              $columns = ['uninterruptible_power_supplies.id', 'title', 'mainImage', 'discount', 'price', 'power'];
              $conditionNumbers = $stockTypesNumbers = [];

              $query = \DB::table('uninterruptible_power_supplies') -> select($columns) -> where('visibility', 1);
    ...
    public function getUninterruptiblePowerSuppliesData()
    {
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $columns = ['uninterruptible_power_supplies.id', 'title', 'mainImage', 'discount', 'price', 'power'];
        $query = \DB::table('uninterruptible_power_supplies') -> select($columns) -> where('visibility', 1) -> where('price', '>=', $productMinPrice) -> where('price', '<=', $productMaxPrice);

        $totalNumOfProducts = $query -> count();
        $data['uninterruptiblePowerSuppliesExist'] = $totalNumOfProducts != 0;
        ...
        if($data['uninterruptiblePowerSuppliesExist'])
        {
          ...
          $data['uninterruptiblePowerSupplies'] = $query -> take($numOfProductsToView) -> get();

          foreach($data['uninterruptiblePowerSupplies'] as $key => $value) $data['uninterruptiblePowerSupplies'][$key] -> newPrice = $value -> price - $value -> discount;
          ...
          $data['uninterruptiblePowerSupplies'] = $data['uninterruptiblePowerSupplies'] -> chunk($chunkSize);
        }
      }
    ...
    public function getUninterruptiblePowerSupplyData($id, $seoFields)
    {
      $numOfProductsToView = 12;
      $pricePart = 0.2;

      $columns = ['uninterruptible_power_supplies.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
      ...
      {
        $seoFields -> description = $data['uninterruptiblePowerSupply'] -> seoDescription;
        $seoFields -> keywords = $data['uninterruptiblePowerSupply'] -> seoKeywords;
        $seoFields -> title = $data['uninterruptiblePowerSupply'] -> title;
        ...
        $data['uninterruptiblePowerSupply'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
