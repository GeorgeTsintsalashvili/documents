...
class VideoCard extends Model
{
        ...
        if(in_array($numOfProductsToView, $viewSupportedValues))
        {
          ...
          if($priceFromIsInRange && $priceToIsInRange)
          {
            ...
            if($memoryCapacityExists && $gpuManufacturersExist && $videoCardsManufacturersExist && $memoryTypesExist && $memoryBandwidthExists && $stockTypeExists && $conditionExists)
            {
              ...
              $columns = ['video_cards.id', 'title', 'mainImage', 'discount', 'price', 'gpuTitle', 'typeTitle', 'memory', 'memoryBandwidth'];
    ...
    public function getVideoCardsData()
    {
      ...
      $data['configuration']['productPriceRange'] = \DB::table('price_configurations') -> select(['videoCardMinPrice', 'videoCardMaxPrice']) -> get() -> first();
      ...
      if($data['configuration']['productPriceRangeExists'])
      {
        ...
        $totalNumberOfVideoCards = $query -> count();
        $data['videoCardsExist'] = $totalNumberOfVideoCards != 0;
        ...
        if($data['videoCardsExist'])
        {
          ...
          $data['videoCards'] = $query -> take($numOfProductsToView) -> get();
          ...
          $data['videoCards'] = $data['videoCards'] -> chunk($chunkSize);
        }
      }
    ...
    public function getVideoCardData($id, $seoFields)
    {
      $numOfProductsToView = 12;
      $pricePart = 0.2;

      $columns = ['video_cards.id', 'code', 'title', 'mainImage', 'discount', 'price', 'description', 'warrantyDuration', 'warrantyId', 'stockTypeId', 'conditionId', 'quantity', 'seoDescription', 'seoKeywords'];
      ...
      {
        $seoFields -> description = $data['videoCard'] -> seoDescription;
        $seoFields -> keywords = $data['videoCard'] -> seoKeywords;
        $seoFields -> title = $data['videoCard'] -> title;
        ...
        $data['videoCard'] -> categoryId = BaseModel::getTableAliasByModelName(self::class);
