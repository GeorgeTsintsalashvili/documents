...
@if($data['productsExist'])
  ...
  @foreach($data['products'] as $product)
   ...
   <li>
    <a class="col-sm-12" href="/{{ $product -> pathPart }}/{{ $product -> id }}" target="_blank">
     ...
     <div class="col-sm-4 search-result-price-block">
       <span class="col-sm-6 search-result-price">₾ {{ $product -> price }}</span>
       @if($product -> discount != 0)
        <span class="col-sm-6 search-result-discount">₾ {{ $product -> price + $product -> discount }}</span>
       @endif
     </div>
    </a>
   </li>
