  ...
  <div class="col-xs-12">
  	<div class="panel panel-default">
  		<div class="panel-heading">
  			<h4 class="font-mtavruli">განცხადებები</h4>
  		</div>
              ...
              <th class="font-4">S-VIP</th>
  				...
  				@foreach($statements as $value)
  					...
  					<tr data-record-id="{{ $value -> id }}">
  						...
  						<!--- identifiers field --->
  						<td class="col-sm-4">
  							<input data-parameter-name="identifiers" type="text" value="{{ $value -> identifiers }}" class="tokenfield no-border transparent-bg-color font-4 field">
  						</td>

              <!--- status update schedule field ---->
              <td class="col-sm-2">
                <input data-parameter-name="updateSchedule" type="text" value="{{ $value -> updateSchedule }}" class="no-border transparent-bg-color font-4 field">
              </td>
              ...
              <td class="col-sm-2">
                <select data-parameter-name="categoryId" class="wpr-100 record-update-data-list field">
                  ...
                  @foreach($categories as $category)
                    <option class="font-4" {{ $category -> id == $value -> categoryId ? "selected" : null }} value="{{ $category -> id }}">{{ $category -> categoryTitle }}</option>
                  @endforeach
        ...
        <div data-page-key="{{ $statementsPageKey }}" data-current-page="{{ $statementsPaginator -> currentPage }}" class="clearfix pagination-column col-sm-12">
          ...
          <select class="control-data-list col-sm-2 p-n mr20" data-control-parameter-name="{{ $categoriesKey }}">
            <option class="font-4 control-field" value="0">ყველა კატეგორია</option>
             @foreach($categories as $category)
               <option value="{{ $category -> id }}" {{ $selectedCategoryId == $category -> id ? "selected" : null }} class="font-4"> {{ $category -> categoryTitle }} </option>
             @endforeach
          </select>
          ...
          <ul class="pagination mt0 mb20 col-sm-8">
  		 ...
  		 <form action="{{ route('statementStore') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
  			<div class="form-group">
  				...
  				<div class="col-sm-4">
  					<input name="identifiers" type="text" class="form-control font-4 tokenfield" placeholder="აიდები">
  				</div>

          <div class="col-sm-3">
            <input name="updateSchedule" type="text" class="form-control font-4" placeholder="განრიგი">
          </div>

          <div class="col-sm-2">
          <select class="form-select select2-offscreen" name="categoryId" style="width: 100%">
            <option class="font-4" value="0" selected>კატეგორია</option>

            @foreach($categories as $category)
             <option class="font-4" value="{{ $category -> id }}">{{ $category -> categoryTitle }}</option>
            @endforeach
...
<div class="row">
  <div class="col-xs-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4 class="font-mtavruli">კატეგორიები</h4>
      </div>
      <div class="panel-body">
        <table class="table table-hover mb1" data-update-route="{{ route('categoryUpdate') }}">
          ...
          @foreach($categories as $value)
            ...
            <tr data-record-id="{{ $value -> id }}">
              ...
              <!--- title field --->
              <td>
                <input data-parameter-name="categoryTitle" type="text" value="{{ $value -> categoryTitle }}" class="no-border transparent-bg-color font-4 field">
              </td>

              <!--- identifier field ---->
              <td>
                <input data-parameter-name="parameterValue" type="text" value="{{ $value -> parameterValue }}" class="no-border transparent-bg-color font-4 field">
              </td>
       ...
       <form action="{{ route('categoryStore') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
        <div class="form-group">
          ...
          <div class="col-sm-5">
            <input name="categoryTitle" type="text" class="form-control font-4" placeholder="დასახელება">
          </div>
          <div class="col-sm-5">
            <input name="parameterValue" type="text" class="form-control font-4" placeholder="იდენტიფიკატორი">
          </div>
        </div>
       </form>
    ...
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4 class="font-mtavruli">სეანსის ქუქი მონაცემები</h4>
      </div>
      <div class="panel-body">
       ...
       <form action="{{ route('sessionCookieUpdate') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
        <div class="form-group">
					<div class="col-sm-12">
					 <textarea name="sessionText" rows="14" class="form-control font-4" style="resize: vertical">{{ $statementsData -> sessionText }}</textarea>
					</div>
        </div>
        <div class="form-group">
          <div class="col-sm-2">
