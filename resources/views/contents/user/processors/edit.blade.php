    ...
    <div class="tab-pane active" id="editor">
      ...
      <form class="form-horizontal record-update-form" method="POST" action="{{ route('cpuUpdate') }}">
          ...
          <div class="form-group">
             ...
             <label class="col-sm-1 control-label font-4">სოკეტი</label>
             <div class="col-sm-2">
              <select name="socketId" class="edit-page-list wpr-100">
               @foreach($sockets as $value)
                 @if($value -> id == $product -> socketId)
                   <option data-chipsets-identifiers="{{ $value -> chipsetsIdentifiers }}" data-chipsets-titles="{{ $value -> chipsetsTitles }}" selected class="font-4" value="{{ $value -> id }}">{{ $value -> socketTitle }}</option>
                 @else
                   <option data-chipsets-identifiers="{{ $value -> chipsetsIdentifiers }}" data-chipsets-titles="{{ $value -> chipsetsTitles }}" class="font-4" value="{{ $value -> id }}">{{ $value -> socketTitle }}</option>
                 @endif
               @endforeach
              </select>
             </div>

             <label class="col-sm-1 control-label font-4">CPU TCP</label>
             <div class="col-sm-2">
              <select name="technologyProcessId" class="edit-page-list wpr-100">
               @foreach($technologyProcesses as $value)
                 @if($value -> id == $product -> technologyProcessId)
                   <option selected class="font-4" value="{{ $value -> id }}">{{ $value -> size }}</option>
                 @else
                   <option class="font-4" value="{{ $value -> id }}">{{ $value -> size }}</option>
                 @endif
               @endforeach
             </select>
            </div>
              ...
              <select name="manufacturerId" class="edit-page-list wpr-100">
               @foreach($manufacturers as $value)
                 @if($value -> id == $product -> manufacturerId)
                   <option selected class="font-4" value="{{ $value -> id }}">{{ $value -> manufacturerTitle }}</option>
                 @else
                   <option class="font-4" value="{{ $value -> id }}">{{ $value -> manufacturerTitle }}</option>
                 @endif
               @endforeach
             </select>
            </div>
           ...
           <label class="col-sm-1 control-label font-4">სიხშირე</label>
           <div class="col-sm-2">
            <input type="text" name="clockSpeed" class="form-control" value="{{ $product -> clockSpeed }}">
           </div>

           <label class="col-sm-1 control-label font-4">ბირთვები</label>
           <div class="col-sm-2">
              <input type="text" name="cores" class="form-control" value="{{ $product -> cores }}">
           </div>
           ...
           <div class="form-group">
             ...
             <label class="col-sm-1 control-label font-4">ჩიპსეტი</label>
             <div class="col-sm-5">
              <select multiple id="chipsets-select" class="form-select wpr-100 populate">

                @foreach($chipsets as $chipset)
                  <option data-socket-id="{{ $chipset -> socketId }}" class="font-4" selected value="{{ $chipset -> id }}">{{ $chipset -> chipsetTitle }}</option>
                @endforeach

                @foreach($restOfChipsets as $chipset)
                  <option data-socket-id="{{ $chipset -> socketId }}" class="font-4" value="{{ $chipset -> id }}">{{ $chipset -> chipsetTitle }}</option>
                @endforeach

              </select>

              <input name="chipsetId" type="hidden" id="chipsets">
