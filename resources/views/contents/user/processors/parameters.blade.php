...
<!--- sockets --->
  	...
  	<div class="col-xs-6">
  		<div class="panel panel-default">
  			<div class="panel-heading">
  				<h4 class="font-mtavruli">სოკეტები</h4>
  			</div>
            ...
            @foreach($sockets as $value)
                ...
                <!--- title field --->
  							<td>
  								<input data-parameter-name="socketTitle" type="text" value="{{ $value -> socketTitle }}" class="no-border transparent-bg-color font-4 field">
  							</td>
         ...
         <form action="{{ route('socketStore') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
            ...
            <div class="col-sm-2">
              <input type="submit" class="btn-primary btn font-4" value="დამატება">
   					...
   					<div class="col-sm-3">
   						<input name="socketTitle" type="text" class="form-control font-4" placeholder="დასახელება">
   					</div>
  					<div class="col-sm-2">
  ...
  <div class="col-xs-6">
  	<div class="panel panel-default">
  		<div class="panel-heading">
  			<h4 class="font-mtavruli">ჩიპსეტები</h4>
  		</div>
  		<div class="panel-body">
              ...
              <th>სოკეტი</th>
  				...
  				@foreach($chipsets as $value)
  					...
  					<tr data-record-id="{{ $value -> id }}">
  						...
  						<!--- title field ---->
  						<td>
  							<input data-parameter-name="chipsetTitle" type="text" value="{{ $value -> chipsetTitle }}" class="no-border transparent-bg-color font-4 field">
  						</td>
              ...
              <td>
								<select data-parameter-name="socketId" class="record-update-data-list field" style="width: 100%">
									@foreach($sockets as $socket)
										<option class="font-4" {{ $socket -> id == $value -> socketId ? "selected" : null }} value="{{ $socket -> id }}">{{ $socket -> socketTitle }}</option>
									@endforeach
								</select>
							</td>
        ...
        <div data-page-key="{{ $chipsetsPageKey }}" data-current-page="{{ $chipsetsPaginator -> currentPage }}" class="clearfix pagination-column col-sm-12">
          ...
          <select class="control-data-list col-sm-4 p-n mr20" data-control-parameter-name="{{ $socketsKey }}">
            <option class="font-4" value="0">ყველა სოკეტი</option>
             @foreach($sockets as $socket)
               <option value="{{ $socket -> id }}" {{ $selectedSocketId == $socket -> id ? "selected" : null }} class="font-4"> {{ $socket -> socketTitle }} </option>
             @endforeach
          </select>
          ...
          <ul class="pagination mt0 mb20 col-sm-7">
  		 ...
  		 <form action="{{ route('chipsetStore') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
  			<div class="form-group">
  				...
  				<div class="col-sm-3">
  					<input name="chipsetTitle" type="text" class="form-control font-4" placeholder="დასახელება">
  				</div>
          <div class="col-sm-4">
          <select name="socketId" class="form-select" style="width: 100%">
            <option class="font-4" value="0" selected>აირჩიეთ სოკეტი</option>
             @foreach($sockets as $socket)
              <option class="font-4" value="{{ $socket -> id }}">{{ $socket -> socketTitle }}</option>
             @endforeach
          </select>
          </div>
  			</div>
...
<div class="row">
  <div class="col-xs-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <h4 class="font-mtavruli">სისტემები</h4>
      </div>
      <div class="panel-body">
        <table class="table table-hover mb1" data-update-route="{{ route('systemUpdate') }}">
          ...
          @foreach($systems as $value)
            ...
            <tr data-record-id="{{ $value -> id }}">
              ...
              <!--- title field --->
              <td>
                <input data-parameter-name="homePageTitle" type="text" value="{{ $value -> homePageTitle }}" class="no-border transparent-bg-color font-4 field">
              </td>

              <!--- homepage title field --->
              <td>
                <input data-parameter-name="seriesTitle" type="text" value="{{ $value -> seriesTitle }}" class="no-border transparent-bg-color font-4 field">
              </td>
       ...
       <form action="{{ route('systemStore') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
        <div class="form-group">
          ...
          <div class="col-sm-4">
            <input name="homePageTitle" type="text" class="form-control font-4" placeholder="სათაური სისტემებში">
          </div>
          <div class="col-sm-4">
            <input name="seriesTitle" type="text" class="form-control font-4" placeholder="სათაური პროცესორებში">
          </div>
          <div class="col-sm-2">
  ...
  <div class="col-xs-6">
  	<div class="panel panel-default">
  		<div class="panel-heading">
  			<h4 class="font-mtavruli">ტექნოლოგიური პროცესები</h4>
  		</div>
  		<div class="panel-body">
  			<table class="table table-hover mb1" data-update-route="{{ route('tcpUpdate') }}">
  				...
  				@foreach($technologyProcesses as $value)
  					...
  					<tr data-record-id="{{ $value -> id }}">
  						...
  						<!--- title field --->
  						<td>
  							<input data-parameter-name="size" type="text" value="{{ $value -> size }}" class="no-border transparent-bg-color font-4 field">
  						</td>
  		 ...
  		 <form action="{{ route('tcpStore') }}" method="POST" class="form-horizontal row-border data-form" enctype="multipart/form-data">
  			<div class="form-group">
  				...
  				<div class="col-sm-4">
  					<input name="size" type="text" class="form-control font-4" placeholder="აკრიფეთ დასახელება">
  				</div>
  			</div>
  		</form>
